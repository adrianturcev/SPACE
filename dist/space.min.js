...
`)&&(s.childNodes.length==0?s.appendChild(e[t].childNodes[a]):s.insertBefore(e[t].childNodes[a],s.childNodes[0]));e[t].firstElementChild?e[t].insertBefore(s,e[t].firstElementChild):e[t].appendChild(s)}}addLineNumbers(e,n,t,r=0){let s=this;for(let a=0,f=e.length;a<f;a++){let c=document.createElement("div");c.className="damon-line-number";let u=s.damon.damonToMap(n),g=e[a].dataset.graphArbo.split("-"),l=[];for(let h=0,p=g.length;h<p;h++)typeof u=="object"&&u!==null&&!Array.isArray(u)&&u instanceof Map&&u.constructor===Map?(l.push(Array.from(u.keys())[g[h]]),u=l[l.length-1]):(l.push(g[h]),u=u[g[h]]);c.textContent=s.damon.getRangeFromPath(n,l)[0][0]+1+r,c.id="damonLine"+c.textContent,e[a].setAttribute("aria-labelledBy",c.id),c.style.left="7px",c.style.top=t.scrollTop+e[a].firstElementChild.getBoundingClientRect().top-t.getBoundingClientRect().top+"px",t.appendChild(c)}}}});var Br=se((jn,Fr)=>{var Zn=sn(lt());Fr.exports=function(i){i.update(),i.historyUpdate(),window.addEventListener("load",e),i.editor.addEventListener("focusin",n),i.editor.addEventListener("focusout",t),i.gutter.addEventListener("click",r),document.addEventListener("mousedown",a),document.addEventListener("mousemove",f),document.addEventListener("mouseup",c),document.addEventListener("selectionchange",u),i.textarea.addEventListener("beforeinput",g),i.textarea.addEventListener("keydown",l),i.textarea.addEventListener("cut",h),i.textarea.addEventListener("copy",p),i.textarea.addEventListener("paste",y);function e(b){i.overlay.firstElementChild.innerHTML="",i.gutter.innerHTML="",i.update(),i.updateCurrentLine(),i.initialized=!0}function n(b){i.utils.addDataState(i.editor,"focused")}function t(b){i.editor.contains(b.target)||i.utils.removeDataState(i.editor,"focused")}function r(b){(Array.from(i.gutter.children).indexOf(b.target)>-1||Array.from(i.gutter.children).indexOf(b.target.parentNode)>-1)&&i.setCarretAt(parseInt(b.target.textContent.trim())-1)}var s;function a(b){if(b.target==i.container){s=b.target,b.preventDefault();let E=i.gutter.children,x="0";for(let C=0,T=E.length;C<T;C++)b.clientY>E[C].getBoundingClientRect().y&&(x=E[C].textContent.trim());x-1<0&&(x=1),i.setCarretAt(x-1),i.textarea.focus()}}function f(b){if(s==i.container&&b.buttons==1){let E=i.gutter.children,x="0";for(let C=0,T=E.length;C<T;C++)b.clientY>E[C].getBoundingClientRect().y&&(x=E[C].textContent.trim());i.textarea.focus(),i.textarea.setSelectionRange(i.textarea.value.split(`
`).slice(0,x).join().length,i.textarea.value.length,"backward")}}function c(b){s==i.container&&b.target==i.textarea&&(i.textarea.selectionEnd=i.textarea.value.length,i.textarea.selectionStart=i.textarea.value.length,i.textarea.focus()),s=void 0}function u(b){document.activeElement===i.textarea&&(i.updateCaret(),i.updateCurrentLine(),i.updateHorizontalScroll(),i.lastSelection=i.getTextareaSelection())}function g(b){i.lastTextareaValue=i.textarea.value}function l(b){if(!b.ctrlKey&&!b.shiftKey&&!b.altKey){if(b.key==" "&&i.textarea.selectionStart==i.textarea.selectionEnd&&(!/\n/.test(i.textarea.value.slice(0,i.textarea.selectionStart))&&(/ +$/.test(i.textarea.value.slice(0,i.textarea.selectionStart))||i.textarea.selectionStart===0)||/\n +$/.test(i.textarea.value.slice(0,i.textarea.selectionStart))||i.textarea.selectionStart<i.textarea.value.length&&(/\n +\n$/.test(i.textarea.value.slice(0,i.textarea.selectionStart+1))||/\n$/.test(i.textarea.value.slice(0,i.textarea.selectionStart)))||i.textarea.value.length==i.textarea.selectionStart&&(/\n +$/.test(i.textarea.value)||/\n$/.test(i.textarea.value)))){b.preventDefault();let E=i.textarea.selectionStart+4,x=i.textarea.value.slice(0,i.textarea.selectionStart)+"    "+i.textarea.value.slice(i.textarea.selectionEnd);i.setTextarea(x,E),d()}if(b.key=="Enter"){let E=i.textarea.value,x=E.split(`
`),C=i.getSelectionStartCoordinates(),T=C[0],m=C[1],o=x[T];if(/^    /.test(o)){if(b.preventDefault(),m%4!==0&&m<o.match(/^(    )+/)[0].length){for(let k=m;0<=k;k--)if(k%4==0){i.textarea.selectionStart==i.textarea.selectionEnd&&(i.textarea.selectionEnd=x.slice(0,T).join().length+k),i.textarea.selectionStart=x.slice(0,T).join().length+k,m=k;break}}let L=0;m>0&&(L=o.slice(0,m).match(/^(    )+/)[0].length);let w=i.textarea.selectionStart,_=i.textarea.value.slice(i.textarea.selectionEnd);_.split(`
`)[0].trim()==""&&(_=`
`+_.split(`
`).slice(1).join(`
`));let D=i.textarea.value.slice(0,i.textarea.selectionStart)+`
`+" ".repeat(L)+_;i.setTextarea(D,w+L+1),d()}}}else if(!b.ctrlKey&&b.shiftKey&&!b.altKey)b.key==" "&&i.textarea.selectionStart==i.textarea.selectionEnd;else if(b.ctrlKey&&!b.shiftKey&&!b.altKey){if(b.key=="Backspace"){let E=i.textarea.value.split(`
`),x=i.getSelectionStartCoordinates(),C=E[x[0]],T=C.slice(0,x[1]);if(i.textarea.selectionEnd==i.textarea.selectionStart&&/^ +$/.test(T)){b.preventDefault();let m=0;T.length%4==0?(E[x[0]]=C.slice(4),m=4):(E[x[0]]=C.slice(T.length%4),m=T.length%4),i.setTextarea(E.join(`
`)),d(),i.setCarretAt(x[0],x[1]-m)}}if(b.key==" "){let E=i.textarea.value.slice(0,i.textarea.selectionStart)+"	"+i.textarea.value.slice(i.textarea.selectionEnd);i.setTextarea(i.formatIndentation(E),i.textarea.selectionEnd+1),d()}if(b.key=="]"){b.preventDefault();let E=i.textarea.value.split(`
...
`),T,m),d()}if(b.key=="["){let E=i.textarea.value,x=i.textarea.value.split(`
...
`),m,o),d()}if(b.key=="Enter"){let E=i.textarea.value.split(`
...
`+E.slice(x+1).join(`
`)),d(),i.setCarretAt(x+1)}if(b.key=="z"&&(b.preventDefault(),i.historyPosition>1)){i.historyPosition-=1;let E=i.history[i.historyPosition-1];i.setTextarea(E[0],E[1]),i.update(),i.virtualScroller.inputHandler(!0),i.textarea.blur(),i.textarea.focus()}if(b.key=="y"&&(b.preventDefault(),i.historyPosition<i.history.length)){i.historyPosition+=1;let E=i.history[i.historyPosition-1];i.setTextarea(E[0],E[1]),i.update(),i.virtualScroller.inputHandler(!0),i.textarea.blur(),i.textarea.focus()}}else if(!b.ctrlKey&&b.shiftKey&&b.altKey)b.key=="f"&&i.setTextarea(i.formatIndentation(i.textarea.value),i.textarea.selectionEnd);else if(!b.ctrlKey&&!b.shiftKey&&b.altKey){if(b.key=="ArrowDown")if(i.getSelectionStartCoordinates()[0]==i.getSelectionEndCoordinates()[0]){let E=i.textarea.value.split(`
...
`),i.textarea.selectionStart+m.length+1,i.textarea.selectionEnd+m.length+1),d()}}else if(b.key=="ArrowUp")if(i.getSelectionStartCoordinates()[0]==i.getSelectionEndCoordinates()[0]){let E=i.textarea.value.split(`
...
`),i.textarea.selectionEnd-m.length-1,i.textarea.selectionStart-m.length-1):i.setTextarea(E.join(`
`),i.textarea.selectionStart-m.length-1,i.textarea.selectionEnd-m.length-1),d()}}}i.debounceLint()}function h(b){if(i.textarea.selectionStart==i.textarea.selectionEnd){b.preventDefault();let E=i.textarea.value.split(`
`),x=i.getCurrentLine(),C=E.slice(0,x).join(`
`).length,T=C+E[x].length+1;i.textarea.setSelectionRange(C,T),i.lastTextareaValue=i.textarea.value,document.execCommand("cut"),i.updateCaret()}}function p(b){if(i.textarea.selectionStart==i.textarea.selectionEnd){b.preventDefault();let E=i.textarea.value.split(`
`),x=i.getCurrentLine(),C=E.slice(0,x).join(`
`).length,T=C+E[x].length+1;i.textarea.setSelectionRange(C,T),document.execCommand("copy")}}function y(b){b.preventDefault();let E=b.clipboardData.getData("text").replace(new RegExp(/\r/g),""),x=i.textarea.value.slice(0,i.textarea.selectionStart),C=i.textarea.value.slice(i.textarea.selectionEnd),T=x+E+C,m=i.textarea.selectionStart+E.length;try{JSON.parse(T),i.setTextarea(i.damon.jsonToDamon(T)),d(),i.updateCaret(),i.debounceLint();return}catch{}let o=T.split(`
